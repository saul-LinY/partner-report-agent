...
function normalizedGitRemote(value: string, canonical = false) {
  const remote = value.trim();
  if (!remote || remote.startsWith("/") || remote.startsWith("file:"))
    return null;
  const scp =
    canonical && remote.includes("://")
      ? null
      : /^(?:[^@/\s]+@)?([^:/\s]+):(.+)$/.exec(remote);
  if (scp?.[1] && scp[2])
...
function gitRemoteIdentity(root: string, canonical = false) {
  try {
    ...
    for (const line of config.split(/\r?\n/)) {
      ...
      const normalized = match?.[1]
        ? normalizedGitRemote(match[1], canonical)
        : null;
...
export function discoverProjectScopes(
  ...
  for (const summary of summaries) {
    ...
    const pathMatch = knownRoots.find((entry) => {
      if (!withinPath(cwd, entry.localRoot)) return false;
      ...
      return (
        !entry.localIdentity ||
        (!local.identitySalt && !currentIdentity) ||
        entry.localIdentity === currentIdentity
      );
    });
    ...
    const localIdentity =
      localRoot === environment.localRoot
        ...
        : projectLocalIdentity(local.scopeSalt, localRoot);
    const scopeKey =
      local.identitySalt && !localIdentity
              ...
              ? [
                  ...new Set([
                    ...
                    ...knownRoots
                      .filter(
                        (entry) =>
                          entry.localIdentity === preferredLocalIdentity,
                      )
...
export function applyScopeResolution(
  ...
  const candidates = discovery.candidates.map((candidate) => {
    ...
    if (
      !scopeKey ||
      !remote.entries.some((entry) => entry.scopeKey === scopeKey)
    )
  ...
  // Retire obsolete local mappings, while keeping central decisions intact.
  // Otherwise a stale key can win the next path lookup and recreate a conflict.
  const remapped = {
    ...local,
    entries: local.entries.map((entry) => {
      const replacement = candidates.find(
        (candidate) =>
          ...
          (candidate.localRoot === entry.localRoot ||
            (candidate.localIdentity &&
              candidate.localIdentity === entry.localIdentity)),
      );
