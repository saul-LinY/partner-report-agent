...
function normalizedGitRemote(value, canonical = false) {
  const remote = value.trim();
  if (!remote || remote.startsWith("/") || remote.startsWith("file:"))
    return null;
  const scp = canonical && remote.includes("://") ? null : /^(?:[^@/\s]+@)?([^:/\s]+):(.+)$/.exec(remote);
  if (scp?.[1] && scp[2])
...
function gitRemoteIdentity(root, canonical = false) {
  try {
    ...
    for (const line of config.split(/\r?\n/)) {
      ...
      const normalized = match?.[1] ? normalizedGitRemote(match[1], canonical) : null;
...
function projectLocalIdentity(scopeSalt, localRoot, canonical = false) {
  const gitRoot = outermostGitRoot(localRoot);
  const remote = gitRoot ? gitRemoteIdentity(gitRoot, canonical) : null;
  let material = remote;
  if (!material) {
    try {
...
function discoverProjectScopes(pluginInstanceId, local, summaries, options = {}) {
  ...
  for (const summary of summaries) {
    ...
    const pathMatch = knownRoots.find((entry) => {
      if (!withinPath2(cwd, entry.localRoot)) return false;
      const currentIdentity = projectLocalIdentity(
        local.scopeSalt,
        entry.localRoot
      );
      return !entry.localIdentity || !local.identitySalt && !currentIdentity || entry.localIdentity === currentIdentity;
    });
    ...
    const scopeKey = local.identitySalt && !localIdentity ? anonymousProjectScopeKey(
      pluginInstanceId,
      local.identitySalt,
      `unverified:${localRoot}`
    ) : inherited?.scopeKey ?? anonymousProjectScopeKey(
      pluginInstanceId,
      local.scopeSalt,
    ...
    discovered.set(scopeKey, {
      ...
      ...local.identitySalt ? {
        identityKey: projectLocalIdentity(
          local.identitySalt,
          preferredLocalRoot,
          true
        ),
        recoveryKeys: preferredLocalIdentity ? [
          .../* @__PURE__ */ new Set([
            ...
            ...knownRoots.filter(
              (entry) => entry.localIdentity === preferredLocalIdentity
...
function applyScopeResolution(local, remote, discovery) {
  const bindings = new Map(
    remote.bindings.map((item) => [item.requestedScopeKey, item.scopeKey])
  );
  const candidates = discovery.candidates.map((candidate) => {
    const scopeKey = bindings.get(candidate.scopeKey);
    if (!scopeKey || !remote.entries.some((entry) => entry.scopeKey === scopeKey))
      throw new Error("PROJECT_SCOPE_RESOLUTION_INCOMPLETE");
    return { ...candidate, scopeKey };
  });
  for (const [id, previous] of discovery.threadScopes) {
    const scopeKey = bindings.get(previous);
    if (!scopeKey) throw new Error("PROJECT_SCOPE_RESOLUTION_INCOMPLETE");
    discovery.threadScopes.set(id, scopeKey);
  }
  const remapped = {
    ...local,
    entries: local.entries.map((entry) => {
      const replacement = candidates.find(
        (candidate) => candidate.scopeKey !== entry.scopeKey && (candidate.localRoot === entry.localRoot || candidate.localIdentity && candidate.localIdentity === entry.localIdentity)
      );
      if (!replacement) return entry;
      const { localIdentity: _identity, ...rest } = entry;
      return { ...rest, localRoot: null };
    })
  };
  return mergeDiscoveredRoots(
    mergeRemoteProjectScope(remapped, remote),
    candidates
  );
}
function threadMayBeRead(summary, local, options = {}, now = /* @__PURE__ */ new Date()) {
  if (local.identitySalt && !local.identityConfirmed) return false;
  if (!scopeIsActive(
...
async function synchronizeLocalProjectScope(remote, inspection = inspectLocalProjectScope(remote.pluginInstanceId)) {
  ...
  const bootstrapped = inspection.state !== "valid" || !inspection.scope.identitySalt;
  const scope = mergeRemoteProjectScope(inspection.scope, remote);
...
async function resolveDiscoveredProjectScopes(local, discovery, periodKey, initialDiscovery) {
  let baseVersion = local.version;
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const remote = await authenticatedRequest("/v2/project-scope/resolve", {
        method: "POST",
        body: JSON.stringify({
          baseVersion,
          periodKey,
...
async function collectStart() {
  ...
  const allThreadDiscovery = discoverProjectScopes(
    config.pluginInstanceId,
    localScope,
    localScope.initialized ? metadataEligible : metadataEligible.filter(
...
function assertCurrentJobPermission(manifest) {
  const summary = manifest.queue.find(
    (item) => item.id === manifest.current?.threadId
  );
  const local = inspectLocalProjectScope(manifest.pluginInstanceId);
  if (!summary || local.state !== "valid" || !threadMayBeRead(summary, local.scope, {
    configuredRoots: configuredProjectRoots(manifest.projects)
  }))
    throw Object.assign(
