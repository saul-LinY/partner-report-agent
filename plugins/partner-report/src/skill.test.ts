...
describe("partner report skill packaging", () => {
  ...
  it("documents the Session-level value screening workflow", () => {
    ...
    expect(skill).toContain("每次定时采集都不得检查、比较或修改");
    expect(skill).toContain("用户提供了新 Prompt 时使用用户原文");
    expect(skill).toContain("恢复默认 Prompt");
    expect(skill).toContain("只更新 `prompt` 字段");
    expect(skill).toContain("重置整个定时任务");
    expect(skill).toContain("不得创建重复任务");
    expect(skill).toContain("任务身份保持不变");
    expect(skill).toContain("仅修改 Prompt 时不得修改 destination");
    expect(skill).toContain("与插件内置默认值不同不构成错误");
    expect(skill).toContain("中等推理");
