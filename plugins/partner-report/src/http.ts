import { loadConfig, loadSecret, saveConfig, saveSecret, type PluginConfig } from "./config.js";
...
async function rawRequest<T>(serverUrl: string, path: string, init: RequestInit = {}, accessToken?: string): Promise<T> {
...
export async function publicRequest<T>(serverUrl: string, path: string, init: RequestInit = {}) {
...
async function refresh(config: PluginConfig) {
  ...
  const tokens = await rawRequest<{ accessToken: string; refreshToken: string; expiresAt: string; pluginInstanceId: string }>(config.serverUrl, "/v1/plugin-bindings/refresh", {
    method: "POST",
...
export async function authenticatedRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (new Date(config.accessExpiresAt).getTime() < Date.now() + 60_000) config = await refresh(config);
  try {
    return await rawRequest<T>(config.serverUrl, path, init, loadSecret(config.pluginInstanceId, "access"));
  } catch (error) {
    if (!(error instanceof HttpError) || error.status !== 401) throw error;
    config = await refresh(config);
    return rawRequest<T>(config.serverUrl, path, init, loadSecret(config.pluginInstanceId, "access"));
  }
}
