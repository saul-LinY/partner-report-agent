...
每次日常采集完成已授权项目的 Session 处理后，Plugin 检查每个已授权项目的整体描述：中台尚无描述时生成候选；已有描述时根据 README、项目清单和顶层目录等说明性材料的语义指纹判断项目定位是否变化，未变化直接复用，变化时才生成新候选。候选描述约 150 字，只说明项目服务对象、用途和核心能力，不出现在项目采集权限卡中，而是与该项目本期工作进展一起陈列在项目工作审核卡片里。Partner 可以用同一个自然语言修改入口调整描述，并通过整张卡片统一审核；接受后候选才成为正式项目描述。团队报告按“项目名、正式项目描述、成员工作进展”展示，不能只展示文件夹名，也不能使用未经工作卡片审核的候选描述。
...
6. 确认 Skill 已创建默认每天北京时间 14:30、运行于新聊天、项目为无、模型为 `gpt-5.6`、推理强度为 `low`、通知所有运行的 Daily Collection Task；这些值可在 Scheduled 面板修改，并授权访问 Report Service 网络域名。
