...
数据中台通过企业自建应用机器人，把项目权限、连接恢复和每周工作卡片私发给对应 Partner。API 进程负责飞书长连接、回调 Inbox 和 Outbox 投递；工作卡片和团队报告生成仍由 Worker 执行。
...
- 连接恢复由插件创建一次性授权请求，用户在已绑定的飞书账号确认后，插件轮换凭据并保留原项目权限。
