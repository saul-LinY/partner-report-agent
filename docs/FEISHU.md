...
身份确认后，系统发送该 Plugin Instance 的首次项目采集范围卡。用户可以逐项或批量允许/拒绝；首次允许立即生效。后续新增项目先保持待审批，不进入 `thread/read`，并在本期原有项目/个人报告审核结束后用一张汇总卡审批，允许结果从下个周期生效。若本期没有原审核卡，Worker 会在提交截止后补发。未点击的项目保持待审批，后续仍可审批。
