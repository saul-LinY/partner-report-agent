...
每周工作卡片生成后按审核版本投递。卡片一次展示一个项目，用户可在卡片内输入自然语言修改意见并重新生成；生成期间同一张卡片展示处理状态，完成后自动更新为新版内容。当前项目只有在用户点击“通过”或“忽略”后才会进入下一项，全部处理完成后冻结审核快照，通过项进入团队报告汇总。
...
- 个人报告和桌面 Widget 不属于飞书协议；自然语言修改仅作用于当前待审核工作卡片，并保留生成版本和用户修改意见用于审计。
