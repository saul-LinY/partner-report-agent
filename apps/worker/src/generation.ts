...
export async function loadPreviousProjectStatus(job: Job, bucket: any) {
  if (!job.partner_id || !bucket.projectId || !job.input_payload.period?.id)
    return null;
  const [previous] = await sql<any[]>`
    select wi.payload, wi.updated_at, rp.period_key from work_items wi
    join report_periods rp on rp.id = wi.period_id
    join report_periods current_period on current_period.id = ${job.input_payload.period.id}
    where wi.tenant_id = ${job.tenant_id} and wi.team_id = ${job.team_id}
  ...
  const [participation] = await sql<any[]>`
    select pp.events, current_period.ends_at from project_participations pp
    join report_periods current_period on current_period.id = ${job.input_payload.period.id}
    where pp.tenant_id = ${job.tenant_id} and pp.team_id = ${job.team_id}
      and pp.partner_id = ${job.partner_id} and pp.project_id = ${bucket.projectId}
  `;
  const confirmedAt =
    previous?.payload.projectStatusConfirmedAt ??
    (previous ? new Date(previous.updated_at).toISOString() : "");
  const manual = (participation?.events ?? [])
    .filter(
      (event: any) =>
        event.type === "milestone" &&
        projectStatusSchema.safeParse(event.projectStatus).success &&
        typeof event.statusConfirmedAt === "string" &&
        event.statusConfirmedAt > confirmedAt &&
        new Date(event.statusConfirmedAt).getTime() <=
          new Date(participation.ends_at).getTime(),
    )
    .sort((a: any, b: any) =>
      b.statusConfirmedAt.localeCompare(a.statusConfirmedAt),
    )[0];
  if (manual)
    return {
      value: manual.projectStatus,
      reason: manual.reason,
      periodKey: null,
    };
  return previous
    ? {
        value: readProjectStatus(previous.payload),
        reason: previous.payload.projectStatusReason ?? "",
        periodKey: previous.period_key,
      }
    : null;
}
    ...
    const batch = await Promise.allSettled(
      projectInputs
        ...
        .map(async (input: any) => {
          ...
          bucket.previousProjectStatus = await loadPreviousProjectStatus(
            job,
            bucket,
          );
          const result = await generateStructured<any>({
            name: "partner_work_item_aggregation",
            schema: (aggregationResultSchema as z.AnyZodObject).extend({
              groups: z.array(
                (
                  (aggregationResultSchema as z.AnyZodObject).shape.groups
                    .element as z.AnyZodObject
                ).extend({
                  projectStatus: projectStatusSchema,
                  projectStatusReason: z.string().trim().min(1).max(500),
                }),
              ),
            }),
