...
const aggregationInstructions = `You aggregate structured Partner work facts across Sessions into reviewable Work Items. Account for every Fact ID exactly once in a group or unassignedFactIds. Prefer explicit project IDs and configured aliases; never invent an ID. Merge only facts that clearly describe the same work thread and keep low-confidence work independent. Preserve uncertainty and completion evidence. Return production metadata {"skillVersion":"partner-report-platform/0.2.0","promptVersion":"2026-08-03.central.v1","schemaVersion":"1.0","producer":"data-platform","modelVersion":"${process.env.OPENAI_MODEL ?? "gpt-5.6-sol"}"}.`;

const reportInstructions = `You generate an individual Partner report from an approved immutable Work Item Snapshot. Include each of the seven required sections exactly once. Every factual claim must cite one or more allowed Work Item IDs. Preferences may change presentation but never facts. State coverage limits plainly and do not invent percentages. Return production metadata {"skillVersion":"partner-report-platform/0.2.0","promptVersion":"2026-08-03.central.v1","schemaVersion":"1.0","producer":"data-platform","modelVersion":"${process.env.OPENAI_MODEL ?? "gpt-5.6-sol"}"}.`;
...
function validateAggregation(job: Job, output: unknown) {
  ...
  const expected = new Set<string>(job.input_payload.facts.map((fact: any) => fact.id));
  ...
  for (const id of expected) if (!used.has(id)) throw new Error(`FACT_COVERAGE_INCOMPLETE:${id}`);
  return result;
}
...
async function applyAggregation(job: Job, output: unknown) {
  ...
  `;
  if (existing.some((item) => item.review_status !== "pending")) throw new Error("REVIEW_ALREADY_STARTED");
...
async function applyReport(job: Job, output: unknown) {
  const result = individualReportResultSchema.parse(output);
  assertReportSemantics(result);
  const allowed = new Set<string>(job.input_payload.workItems.map((item: any) => item.id));
  for (const section of result.sections) for (const claim of section.claims) {
    for (const id of claim.workItemIds) if (!allowed.has(id)) throw new Error(`UNKNOWN_WORK_ITEM_REFERENCE:${id}`);
  }
  ...
  if (!report || ["SUBMITTED", "LOCKED"].includes(report.status)) throw new Error("REPORT_NOT_EDITABLE");
  ...
  await sql.begin(async (tx) => {
    await tx`
      insert into individual_report_versions (
        ...
        ${job.input_payload.sourceChecksum}, ${`partner-report-platform/0.2.0 (${process.env.OPENAI_MODEL ?? "gpt-5.6-sol"})`}
      )
    `;
    await tx`
...
export async function processNextGenerationJob() {
  ...
  try {
    const output = job.type === "AGGREGATE_WORK_ITEMS"
    ...
    const applied = job.type === "AGGREGATE_WORK_ITEMS"
      ? await applyAggregation(job, output)
      : await applyReport(job, output);
    await sql`
    ...
    await sql`
      update agent_jobs set status = ${terminal ? "FAILED" : "RETRY_WAIT"},
        error_code = ${process.env.OPENAI_API_KEY ? "CENTRAL_GENERATION_FAILED" : "MODEL_NOT_CONFIGURED"},
