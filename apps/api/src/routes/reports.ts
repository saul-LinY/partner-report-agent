...
const regenerationSchema = z
  .object({ instruction: z.string().trim().min(2).max(1200) })
  .strict();
...
export async function reportRoutes(app: FastifyInstance) {
  app.get("/v1/individual-reports/:id", async (request) => {
    ...
    return {
      ...
      current:
        versions.find(
          (version) => version.version === report.current_version,
        ) ?? null,
      versions,
    };
  });
  ...
  app.post("/v1/individual-reports/:id/regenerate", async (request) => {
    ...
    const [snapshotRows, templateRows, currentRows] = await Promise.all([
      sql<
        any[]
      >`select * from work_item_snapshots where id = ${report.snapshot_id} and tenant_id = ${actor.tenantId}`,
      sql<any[]>`
      ...
      `,
      sql<any[]>`
        select version, title, summary, markdown, payload
        from individual_report_versions
        where report_id = ${id} and tenant_id = ${actor.tenantId}
          and version = ${report.current_version}
        limit 1
      `,
    ]);
    ...
    const currentReport = currentRows[0];
    if (!currentReport)
      throw new ApiError(
        409,
    ...
    const jobRows = await sql<any[]>`
      insert into agent_jobs (
        ...
        'REGENERATE_INDIVIDUAL_REPORT', ${`report-regenerate:${id}:${key}:${jobId}`},
        ${JSON.stringify({
          schemaVersion: "1.0",
          reportId: id,
          ...
          preferences: { reviewInstruction: input.instruction },
          constraints: {
  ...
  app.post("/v1/individual-reports/:id/return-to-items", async (request) => {
    ...
    const snapshots = await sql<
      any[]
    >`select review_id from work_item_snapshots where id = ${report.snapshot_id}`;
  ...
  app.post("/v1/individual-reports/:id/submit", async (request) => {
    ...
    const { baseVersion } = z
      ...
      .parse(request.body);
    const report = await loadReport(actor, id);
    if (report.current_version !== baseVersion)
      throw new ApiError(
        409,
        "VERSION_CONFLICT",
        "Report 已更新，请刷新后提交。",
      );
    if (report.status !== "REPORT_REVIEW")
      throw new ApiError(
        409,
  ...
  app.get("/v1/individual-reports/:id/download.md", async (request, reply) => {
    ...
    if (!version)
      throw new ApiError(404, "REPORT_VERSION_MISSING", "Report 版本不存在。");
    reply.header("content-type", "text/markdown; charset=utf-8");
    reply.header(
      "content-disposition",
      `attachment; filename="partner-report-v${version.version}.md"`,
    );
